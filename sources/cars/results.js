goog.provide("cars.Results");
goog.require("cars.template.results");
 ...
 */
cars.Results = function (arrResults, passenger, cortinilla, objSearch) {
    //debugger;
...
goog.inherits(cars.Results, goog.ui.Component);

cars.Results.prototype.filters = {};
cars.Results.prototype.filters.airlines = ['*'];
cars.Results.prototype.filters.stops = ['*'];
cars.Results.prototype.filters.arrivalAirPorts = ['*'];
cars.Results.prototype.filters.departureAirports = ['*'];
cars.Results.prototype.filters.arrivalTimes = ['*'];
cars.Results.prototype.filters.departureTimes = ['*'];
cars.Results.prototype.filters.amountCurrencys = ['*'];
cars.Results.prototype.filters.estimatedTimes = ['*'];
cars.Results.prototype.clusterVisibles = 0;
cars.Results.prototype.maxClusterVisibles = 0;
cars.Results.prototype.currentClusterVisible = 0;
cars.Results.prototype.maxClusterRenderPerScrloll = 1;
cars.Results.prototype.lastDivVisiblePosition = 0;
cars.Results.prototype.arrResults = {};

/**
 * Creates an initial DOM representation for the component.
 * @override
 */
cars.Results.prototype.createDom = function () {
    ...
    this.setId("clusters");
    this.contenido = cars.template.results.results();
    element_.innerHTML = this.contenido;
    if (!goog.DEBUG) {
        this.MODULE_RESULTS_CSS = goog.style.installStyles(MODULE_CARS_RESULTS_CSS);
    }
};

cars.Results.prototype.enterDocument = function () {
 ...
 * @override
 */
cars.Results.prototype.exitDocument = function () {
 ...
 * @returns {undefined}
 */
cars.Results.prototype.renderResults = function (from, to) {
    ...
    //        var cluster = goog.dom.htmlToDocumentFragment(cars.template.results.cars({tcAirSearchRSField: field}, null));
    ...
    while (this.lastDivVisiblePosition <= (window.innerHeight + goog.dom.getPageScroll().y) || renderAll) {
        ...
        var cluster = goog.dom.htmlToDocumentFragment(cars.template.results.results({ tcAirSearchRSField: field }, null));
        goog.dom.appendChild(this.clusters, cluster);
        this.currentClusterVisible++;
...
cars.Results.prototype.addEvents = function () {
    //var stopsLink = goog.dom.query("." + goog.getCssName("stops-link"));
    //for (var i = 0; i < stopsLink.length; i++) {
    //    this.getHandler().listen(stopsLink[i], goog.events.EventType.CLICK, this.onSelectStops_);
    //}
    ...
    if (this.btnreturnsearch !== null) {
        this.getHandler().listen(this.btnreturnsearch, goog.events.EventType.CLICK, this.onButtonReturnClick_);
    }

};

cars.Results.prototype.renderOne = function (tcAirSearchRSField) {
    var cluster = goog.dom.htmlToDocumentFragment(cars.template.results.result({ tcAirSearchRSField: tcAirSearchRSField }, null));
    goog.dom.appendChild(this.clusters, cluster);

};

cars.Results.prototype.closePopup = function () {
    this.objPopup.setVisible(false);
};

cars.Results.prototype.onDetailsClick = function (e) {

    debugger;
    this.cortinilla.style.display = "block";

    moduleManager.execOnLoad('cars_details', goog.bind(this.onDetailsLoaded, this));

    //if (typeof (this.popup) === "undefined") {
    //    this.popup = goog.dom.htmlToDocumentFragment(cars.template.results.popup({}, null));
    ...
    //this.popupContent.innerHTML = cars.template.results.popupContent({tcAirItineraryField: itineraryselected}, null);
...
cars.Results.prototype.onDetailsLoaded = function (e) {
    debugger;
    ...
    this.details = new cars.Details(this);
...
cars.Results.prototype.onButtonReturnClick_ = function () {
    this.dispose();
};

cars.Results.prototype.onLoadFilters_ = function () {
    this.divFilters = goog.dom.getElementByClass(goog.getCssName("filters"));
    this.divFilters.innerHTML = cars.template.results.filters({ filters: this.filterOptions }, null);
...
cars.Results.prototype.renderFilters = function () {
    this.divFilters = goog.dom.getElementByClass(goog.getCssName("filters"));
    this.divFilters.innerHTML = cars.template.results.filters({ filters: this.filterOptions }, null);
    this.chkInputs = goog.dom.query("." + goog.getCssName("filters") + " " + "input");
    goog.array.forEach(this.chkInputs, function (obj, index, array) {
...
cars.Results.prototype.onClickFilter_ = function (e) {
    //debugger;
 ...
 * click boton buscar
 */
cars.Results.prototype.onBuyClick_ = function (e) {
    ...
    moduleManager.execOnLoad('cars_register', goog.bind(this.onRegisterLoaded_, this));
 ...
 * @returns {undefined}
 */
cars.Results.prototype.onRegisterLoaded_ = function () {
    debugger;
    ...
    this.register = new cars.Register(this);
    //if (this.objSearch.objRender !== null) {
 ...
 * @returns {undefined}
 */
cars.Results.prototype.setResultados = function (arrResults) {
    //debugger;
    this.arrResults = arrResults;
};

cars.Results.prototype.setFilterOptions = function (objtcAirSearchRSField) {
...
cars.Results.prototype.getObjectValuesAndCount = function (arraySearch) {
...
cars.Results.prototype.setFilterResults = function (objtcAirSearchRSField) {
...
cars.Results.prototype.Filters = function () {
    //debugger;
    var ids = new Array();
...
cars.Results.prototype.orderResults = function (e) {
    //debugger;
...
cars.Results.prototype.setConfiguration = function () {
 ...
 */
moduleManager.setLoaded('cars_results');